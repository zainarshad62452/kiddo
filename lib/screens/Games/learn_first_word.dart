...
class LearnFirstWord extends StatefulWidget {
  @override
  _GameScreenState createState() => _GameScreenState();
}

class _GameScreenState extends State<LearnFirstWord> {
  int selectedCard = -1;
  @override
      ...
      onTap: () => {
        setState(
          () => {
            selectedCard = index,
            Navigator.pushNamed(
                context, learnGameRoutes[index]['routePath'].toString())
          },
        ),
      },
      child: Container(
  ...
  SizedBox buildCards(BuildContext context) {
    return SizedBox(
      ...
      child: ListView.builder(
        itemCount: learnGameList.length,
        itemBuilder: (BuildContext context, int index) {
          return cardsStyle(learnGameList[index]['imagePath'].toString(),
              learnGameList[index]['GameName'].toString(), index);
        },
      ),
    );
  }
