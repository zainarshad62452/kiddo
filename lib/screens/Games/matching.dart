...
class _GameScreenState extends State<MatchingGameScreen> {
  int selectedCard = -1;
  @override
      ...
      onTap: () => {
        setState(
          () => {
            selectedCard = index,
            Navigator.pushNamed(
                context, matchingGameRoutes[index]['routePath'].toString())
          },
        ),
      },
      child: Container(
  ...
  SizedBox buildCards(BuildContext context) {
    return SizedBox(
      ...
      child: ListView.builder(
        itemCount: matchingGameList.length,
        itemBuilder: (BuildContext context, int index) {
          return cardsStyle(matchingGameList[index]['imagePath'].toString(),
              matchingGameList[index]['GameName'].toString(), index);
        },
      ),
    );
  }
